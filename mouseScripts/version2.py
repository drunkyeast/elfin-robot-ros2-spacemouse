...
class SpaceMouseSimpleControl(Node):
    def __init__(self):
        ...
        self.min_interval = 0.03    # 最小调用间隔30ms（提高平滑度）
        self.last_input_time = 0.0  # 上次有输入的时间
        self.stop_timeout = 0.6     # 停止超时时间600ms（适应真实机械臂惯性）
        self.is_moving = False      # 是否正在运动
        self.pending_commands = 0   # 待处理的指令数量
        self.max_pending = 3        # 最大待处理指令数（防止指令堆积）
        
        # 运动平滑参数
        self.command_queue = []     # 指令队列
        self.last_command = None    # 上一个指令
        self.repeat_threshold = 3   # 相同指令重复次数阈值
        self.repeat_count = 0       # 当前指令重复次数
        
        # 创建定时器检查停止
        self.stop_timer = self.create_timer(0.05, self.check_stop_condition)  # 更频繁检查
        
        self.get_logger().info('=== SpaceMouse简单控制节点已启动 (改进版v1) ===')
        ...
        self.get_logger().info('最大待处理指令: {}'.format(self.max_pending))
    ...
    def twist_callback(self, msg):
        ...
        if self.pending_commands >= self.max_pending:
            self.get_logger().debug('指令队列已满，跳过此次输入')
            return
        ...
        if command_data is not None:
            # 检查指令重复性，避免过度发送相同指令
            if command_data == self.last_command:
                self.repeat_count += 1
                # 如果相同指令重复太多次，适当降低频率
                if self.repeat_count > self.repeat_threshold:
                    if (current_time - self.last_call_time) < (self.min_interval * 2):
                        return
            else:
                self.repeat_count = 0
    ...
    def send_teleop_command(self, command_data, axis_name, value):
        ...
        self.get_logger().info(
            f'发送指令: {axis_name} {direction} (值:{value:.3f}, 指令:{command_data}, 队列:{self.pending_commands})'
        )
    ...
    def stop_robot(self):
        ...
        self.get_logger().info('🛑 发送停止指令（超时停止）')
...
def main():
    ...
    try:
        ...
        node.get_logger().info('🚀 改进版特性：更好的真实机械臂支持')
